...
import { EncryptionService, NoOpCryptoAuditLogger } from '../utils/encryption.service';
import { ENCRYPTED_FIELDS } from '../config/encryption.config';
...
describe('Encryption Middleware', () => {
  ...
  beforeAll(() => {
    encryptionService = new EncryptionService(TEST_KEY, new NoOpCryptoAuditLogger());
    middleware = createEncryptionMiddleware(ENCRYPTED_FIELDS, encryptionService);
  });
  ...
  describe('Client model encryption', () => {
    ...
    it('should encrypt Client fields on create', async () => {
      ...
      const data = (capturedArgs as any).data;
    ...
    it('should decrypt Client fields on findMany', async () => {
      ...
      const result = await middleware(params, next) as Array<Record<string, unknown>>;
  ...
  describe('Encrypted data in DB is not readable as plaintext', () => {
    it('should produce encrypted values that do not contain original plaintext', async () => {
      ...
      const next = async (p: Prisma.MiddlewareParams) => {
        capturedData = (p.args as any).data;
        return { id: 1, ...p.args?.data };
      };
      ...
      const encryptedEmail = (capturedData as any).email;
    ...
    it('should not double-encrypt already encrypted values', async () => {
      ...
      expect((capturedData as any).email).toBe(alreadyEncrypted);
    });
  });
  ...
  describe('Upsert operations', () => {
    it('should encrypt fields in both create and update parts of upsert', async () => {
      ...
      expect((capturedArgs as any).create.email).toMatch(/^enc:/);
      expect((capturedArgs as any).update.email).toMatch(/^enc:/);
    });
  });
