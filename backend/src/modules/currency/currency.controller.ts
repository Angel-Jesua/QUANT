...
import { ICreateCurrency, IUpdateCurrency, IRequestContext, ICurrencyResponse } from './currency.types';
import { logErrorContext, logAuditError } from '../../utils/error';
import { AuditAction } from '@prisma/client';

type CurrencyCreateDTO = ICreateCurrency;
type CurrencyUpdateDTO = IUpdateCurrency;
type Currency = ICurrencyResponse;
...
export class CurrencyController {
  ...
  async getAllCurrencies(req: Request, res: Response): Promise<void> {
    try {
      ...
      logErrorContext('currency.getAll.error', error, {
  ...
  async getCurrencyById(req: Request, res: Response): Promise<void> {
    try {
      ...
      logErrorContext('currency.getById.error', error, { id: req.params.id });
      await logAuditError({
        action: AuditAction.update,
        entityType: 'currency',
  ...
  async createCurrency(req: Request, res: Response): Promise<void> {
    try {
      const body = req.body as Partial<CurrencyCreateDTO & { rate?: number | string }>;
      ...
      if (code.length !== 3) errors.code = 'code must be a 3-character string';
      if (!name) errors.name = 'name is required';
      if (!symbol) errors.symbol = 'symbol is required';
      if (!hasExchangeRate) errors.exchangeRate = 'exchangeRate (or rate) is required';
      let exchangeRate: number | string | undefined = undefined;
      if (hasExchangeRate) {
        if (typeof exchangeRateCandidate === 'number') {
          exchangeRate = exchangeRateCandidate;
          if (!(exchangeRateCandidate > 0)) errors.exchangeRate = 'exchangeRate must be > 0';
          ...
          if (v.length === 0) {
            ...
            const num = Number(v);
            if (!(num > 0)) errors.exchangeRate = 'exchangeRate must be > 0';
            exchangeRate = v;
          }
        } else {
      ...
      const payload: ICreateCurrency = {
        code,
        name,
        symbol,
        decimalPlaces:
          typeof body.decimalPlaces === 'number' ? body.decimalPlaces : undefined,
        isBaseCurrency:
          typeof body.isBaseCurrency === 'boolean' ? body.isBaseCurrency : undefined,
        exchangeRate,
        isActive:
      ...
      const safeBody = req?.body as Partial<ICreateCurrency> | undefined;
      ...
      logErrorContext('currency.create.error', error, { code: safeBody?.code, name: safeBody?.name });
      await logAuditError({
  ...
  async updateCurrency(req: Request, res: Response): Promise<void> {
    try {
      ...
      const body = raw as Partial<CurrencyUpdateDTO & { rate?: number | string }>;

      const normalized: IUpdateCurrency = {};
      if (typeof body.code === 'string') normalized.code = body.code.trim().toUpperCase();
      if (typeof body.name === 'string') normalized.name = body.name.trim();
      if (typeof body.symbol === 'string') normalized.symbol = body.symbol.trim();
      if (typeof body.decimalPlaces === 'number') normalized.decimalPlaces = body.decimalPlaces;
      if (typeof body.isBaseCurrency === 'boolean') normalized.isBaseCurrency = body.isBaseCurrency;
      ...
      const idParam = Number.parseInt(req.params.id, 10);
      logErrorContext('currency.update.error', error, { id: idParam });
      await logAuditError({
        action: AuditAction.update,
        entityType: 'currency',
  ...
  async deleteCurrency(req: Request, res: Response): Promise<void> {
    try {
      ...
      const idParam = Number.parseInt(req.params.id, 10);
      logErrorContext('currency.delete.error', error, { id: idParam });
      await logAuditError({
