...
import {
  ...
  IIncomeStatementCategoryTotals,
  IIncomeStatementSummary,
...
export class IncomeStatementService {
  ...
  async getIncomeStatement(
    ...
    const incomeStatementTypes: AccountType[] = ['Ingresos', 'Costos', 'Gastos'];

    // Calculate current period balances
    const currentBalances = await this.calculatePeriodBalances(
      periodStart,
      periodEnd,
    ...
    if (compStart && compEnd) {
      previousBalances = await this.calculatePeriodBalances(
        compStart,
        compEnd,
        incomeStatementTypes,
        includeInactive
    ...
    const entries: IIncomeStatementEntry[] = [];
    const categoryTotals: Record<IncomeStatementCategory, { total: number; previousTotal: number; count: number }> = {
    ...
    for (const account of accounts) {
      const category = getIncomeStatementCategory(account.type);
    ...
    const categories: IIncomeStatementCategoryTotals[] = Object.entries(categoryTotals)
      .map(([key, value]) => {
        const category = key as IncomeStatementCategory;
        const totals: IIncomeStatementCategoryTotals = {
          category,
    ...
    const summary: IIncomeStatementSummary = {
      startDate,
      endDate,
      totalRevenue,
      totalCosts,
