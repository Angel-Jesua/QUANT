 ...
 */
function createEncryptionMiddleware(config, encryptionService) {
    // Build a lookup map for faster field checking
    const encryptedFieldsMap = new Map();
     ...
     */
    function decryptFields(data, fields, model) {
        const result = { ...data };
        for (const field of fields) {
            if (field in result && result[field] !== null && result[field] !== undefined) {
                const value = result[field];
     ...
     */
    function encryptNestedData(data, model, fields) {
     ...
     */
    function decryptResult(result, model, fields) {
        ...
        if (typeof result === 'object') {
            return decryptFields(result, fields, model);
        }
        return result;
    }
    /**
