...
const DECIMAL_ONE = new client_1.Prisma.Decimal(1);
...
function isPositive(dec) {
    return dec.toNumber() > 0;
}
function requireBaseRateEqualsOne(rate) {
    if (!rate.equals(DECIMAL_ONE)) {
        throw new Error('BASE_RATE_NOT_ONE');
    }
}
function requireNonBaseRatePositive(rate) {
    if (!(rate.toNumber() > 0)) {
...
class CurrencyService {
    ...
    async createCurrency(data, requestContext) {
        ...
        const decimalPlaces = typeof data.decimalPlaces === 'number' ? data.decimalPlaces : 2;
        const isBaseCurrency = typeof data.isBaseCurrency === 'boolean' ? data.isBaseCurrency : false;
    ...
    async updateCurrency(id, data, requestContext) {
        // Load current state
        ...
        if (typeof data.symbol === 'string')
            updateData.symbol = data.symbol.trim();
        if (typeof data.decimalPlaces === 'number')
            updateData.decimalPlaces = data.decimalPlaces;
        if (typeof data.isBaseCurrency === 'boolean')
            updateData.isBaseCurrency = data.isBaseCurrency;
        ...
        const isBaseAfter = typeof updateData.isBaseCurrency === 'boolean'
            ? updateData.isBaseCurrency
            : current.isBaseCurrency;
        const isActiveAfter = typeof updateData.isActive === 'boolean'
            ? updateData.isActive
            : current.isActive;
        const rateAfter = exchangeRateDecimal !== undefined ? exchangeRateDecimal : current.exchangeRate;
        // Business validations
        if (isBaseAfter) {
        ...
        const togglingToBaseActive = !current.isBaseCurrency && isBaseAfter === true && isActiveAfter === true;
        const activatingBaseAgain = current.isBaseCurrency === true &&
            current.isActive === false &&
            isActiveAfter === true;
        const changingRateOnBase = current.isBaseCurrency === true && exchangeRateDecimal !== undefined;
        try {
            let updated;
    ...
    async deleteCurrency(id, requestContext) {
        try {
            // Ensure it exists
