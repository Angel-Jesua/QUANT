...
class BalanceSheetService {
     ...
     */
    async calculateBalancesAsOfDate(asOfDate, accountTypes, includeInactive = false) {
    ...
    async getBalanceSheet(query, userId) {
        const { asOfDate, compareDate, includeInactive = false, showZeroBalances = false, } = query;
        // Parse dates
        ...
        const balanceSheetTypes = ['Activo', 'Pasivo', 'Capital'];
        // Calculate current balances
        const currentBalances = await this.calculateBalancesAsOfDate(reportDate, balanceSheetTypes, includeInactive);
        // Calculate previous balances if comparison date provided
        let previousBalances = null;
        if (comparisonDate) {
            previousBalances = await this.calculateBalancesAsOfDate(comparisonDate, balanceSheetTypes, includeInactive);
        }
        ...
        for (const account of accounts) {
            const section = (0, report_types_1.getBalanceSheetSection)(account.type);
            if (!section)
                continue;
        ...
        const sections = Object.entries(sectionTotals).map(([key, value]) => {
            const section = key;
            const totals = {
                section,
                sectionName: (0, report_types_1.getSectionDisplayName)(section),
