 ...
 */
function buildTestApp(authImpl) {
    jest.isolateModules(() => {
        jest.resetModules();
    });
    jest.isolateModules(() => {
        ...
        global.__clients_app__ = app;
    });
    return global.__clients_app__;
}
describe('Client Endpoints - Router smoke tests', () => {
    ...
    beforeEach(() => {
        ...
        const authWithUser = (req, _res, next) => {
            req.userId = 100;
    ...
    describe('POST /api/clients - Create Client', () => {
        ...
        it('returns 201 with Location header and success payload for valid data', async () => {
            ...
            expect(mockClientService.createClient).toHaveBeenCalledWith(expect.objectContaining({
                clientCode: 'CLI-001',
                name: 'Acme Corp',
                currencyId: 1,
    ...
    describe('GET /api/clients - List Clients', () => {
        ...
        it('returns 401 when userId is missing', async () => {
            const authWithoutUser = (_req, _res, next) => {
                // Do not attach userId; controller should return 401
                next();
            };
            const appNoUser = buildTestApp(authWithoutUser);
            const res = await (0, supertest_1.default)(appNoUser).get('/api/clients').expect(401);
            expect(res.body.success).toBe(false);
    ...
    describe('GET /api/clients/:id - Get Client by ID', () => {
        it('returns 200 for owned client', async () => {
            ...
            const res = await (0, supertest_1.default)(app).get('/api/clients/77').expect(200);
            expect(res.body.success).toBe(true);
        ...
        it('returns 404 for non-owned client', async () => {
            ...
            const res = await (0, supertest_1.default)(app).get('/api/clients/88').expect(404);
            expect(res.body.success).toBe(false);
            expect(res.body.error).toEqual(expect.objectContaining({ message: 'Cliente no encontrado' }));
        });
        it('returns 400 for invalid id', async () => {
            const res = await (0, supertest_1.default)(app).get('/api/clients/abc').expect(400);
            expect(res.body.success).toBe(false);
        });
    });
    describe('PUT /api/clients/:id - Update Client', () => {
        ...
        it('returns 404 when client not found or not owned', async () => {
            ...
            const res = await (0, supertest_1.default)(app).put('/api/clients/99').send({ name: 'X' }).expect(404);
            expect(res.body.success).toBe(false);
        });
        it('returns 403 for invalid operation (clientCode change)', async () => {
            ...
            const res = await (0, supertest_1.default)(app).put('/api/clients/50').send({ clientCode: 'NEW-50' }).expect(403);
            expect(res.body.success).toBe(false);
        ...
        it('returns 409 for duplicate tax id', async () => {
            ...
            const res = await (0, supertest_1.default)(app).put('/api/clients/60').send({ taxId: 'NEW-TAX' }).expect(409);
            expect(res.body.success).toBe(false);
        ...
        it('returns 400 for invalid currencyId', async () => {
            ...
            const res = await (0, supertest_1.default)(app).put('/api/clients/70').send({ currencyId: -1 }).expect(400);
            expect(res.body.success).toBe(false);
        ...
        it('returns 400 for invalid creditLimit', async () => {
            ...
            const res = await (0, supertest_1.default)(app).put('/api/clients/71').send({ creditLimit: -5 }).expect(400);
            expect(res.body.success).toBe(false);
            expect(mockClientService.updateClient).not.toHaveBeenCalled();
        });
    });
    describe('DELETE /api/clients/:id - Delete Client', () => {
        it('returns 204 for owned client deletion', async () => {
            ...
            const res = await (0, supertest_1.default)(app).delete('/api/clients/31').expect(204);
            expect(res.text).toBe('');
        });
        it('returns 404 when client not found', async () => {
            mockClientService.getClientById.mockResolvedValue(null);
            const res = await (0, supertest_1.default)(app).delete('/api/clients/41').expect(404);
            expect(res.body.success).toBe(false);
        });
        it('returns 401 when userId is missing', async () => {
            const authWithoutUser = (_req, _res, next) => {
            ...
            const appNoUser = buildTestApp(authWithoutUser);
            const res = await (0, supertest_1.default)(appNoUser).delete('/api/clients/31').expect(401);
            expect(res.body.success).toBe(false);
            expect(res.body.error).toEqual(expect.objectContaining({ message: 'Unauthorized' }));
        });
    });
