...
exports.importJournalEntries = importJournalEntries;
...
function detectSheetType(data) {
    ...
    const matchCount = structuredHeaders.filter(h => headers.some(header => header.includes(h))).length;
    return matchCount >= 4 ? 'structured' : 'voucher';
}
/**
 * Parse structured sheet (tabular format)
 */
function parseStructuredSheet(sheetName, data) {
    const entries = [];
 ...
 */
function parseVoucherSheet(sheetName, data) {
    const entry = {
        sheetName,
        entryDate: new Date(),
...
async function findAccountByCode(code) {
    const prisma = (0, journal_repository_1.getPrismaClient)();
 ...
 */
async function importJournalEntries(buffer, options, userId) {
    ...
    if (options.selectedSheets && options.selectedSheets.length > 0) {
        entriesToImport = entriesToImport.filter(e => options.selectedSheets.includes(e.sheetName));
    }
    ...
    for (const entry of entriesToImport) {
        try {
            ...
            const created = await (0, journal_service_1.createJournalEntry)({
            ...
            if (options.autoPost) {
                await (0, journal_service_1.postJournalEntry)(created.id, userId);
            }
            result.imported++;
        }
        catch (error) {
